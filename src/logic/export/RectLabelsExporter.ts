...
export class RectLabelsExporter {
    ...
    private static async exportAsYOLO(): Promise<void> {
     ...
     // Store Database for rect
    const annotations = [];
    console.log("imag data ",imagesData[i],"file content ",fileContent)
    const image = ImageRepository.getById(imagesData[i].id);
    console.log("ahlem",image)
    for (let j = 0; j < imagesData[i].labelRects.length; j++) {
      annotations.push({
        label: LabelsSelector.getLabelNameById(imagesData[i].labelRects[j].labelId).name,
        segmentation: [{"x":imagesData[i].labelRects[j].rect.x,
                        "Y":imagesData[i].labelRects[j].rect.y}],
        width:imagesData[i].labelRects[j].rect.width,
        height:imagesData[i].labelRects[j].rect.height,
        type:"rect",
      });
    }
